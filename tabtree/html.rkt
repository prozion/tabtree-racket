...
(define-catch (fullpath-to-data tabtree-root tabtree-path)
  (let* (
        (tabtree-path (string-split (->string tabtree-path) "."))
    ...
    (else
      (let* (
            ...
            (result (for/fold
                      ...
                      ((snippet snippets))
                      (let* (
                            (snippet-lst (string-split snippet ":"))
                            (snippet-varnames (string-split (first snippet-lst) ","))
  ...
  (let* (
        ...
        (tabtrees (for/fold
                    ...
                    (let* ((collection-lst (string-split collection "//"))
                          ...
                          (tabtree-path (string-split tabtree-path "."))
        ...
        (function-part (cond
                          ...
                          (else identity)))
        (parameter-values (map (λ (collection-path tabtree) (get-$1 (rest (string-split collection-path ".")) tabtree)) collection-paths tabtrees))
...
(define-catch (process-html-template filepath #:tabtree-root (tabtree-root "") #:namespace (namespace #f))
  (define-catch (get-cycle-object lst)
    (let* (
          ...
          (collection-part (string-split (second lst) ":"))
          ...
          (snippet-lambda (if snippet-lambda
                            ...
                            identity))
          (cycled-data (get-$3-items (cdr tabtree-path) tabtree))
